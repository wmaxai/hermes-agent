...
_CONTENT_POLICY_BLOCKED_PATTERNS = (
    ...
    # Alibaba Cloud DashScope input content inspection. A long-lived session can
    # carry one flagged turn that DashScope re-inspects on *replayed history*,
    # so every later request fails deterministically with HTTP 400
    # ``InternalError.Algo.DataInspectionFailed`` (code ``data_inspection_failed``).
    # Without these patterns the 400 falls through to the generic non-retryable /
    # context-overflow path and users see the misleading "model provider failed
    # after retries" instead of the provider-block explanation with fallback guidance.
