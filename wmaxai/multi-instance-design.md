...
| 模式 | 特性 | 策略 |
|---|---|---|
| **webhook** | 无状态入站，HTTP 请求即路由 | **不需要粘性路由**。入站即落库 + 租约决定谁执行 |
| **长连接（WS/飞书）** | 连接有主，谁连谁收 | 需要**粘性**（连接层解决）或**入站转发** |
...
#### 4.3 补 fencing token（防脑裂）

**这是多副本下唯一可能写坏数据的地方**，必须解决。

**问题场景**：pod A 网络分区，租约过期，本地进程还活着 → 它以为自己还持有租约继续跑 → 此时 pod B 已合法接管 → **两个实例同时写同一份会话历史**。

**方案**：引入单调递增的 fencing token
1. 租约表增加 `fence BIGINT`，每次接管时 `fence = fence + 1`
2. 持有者拿到租约时记录 `fence` 值，**每次写消息时带上**
3. 写入时校验：`UPDATE ... WHERE fence >= $持有值`，不满足则拒绝写入并终止该 turn

现有 holder 字符串里已经有 `pid=...:turn=...:platform=...` 结构，具备改造基础，但缺单调序号。
...
**问题**：跨机器时 `_compression_lock_holder_process_is_dead` 失效，故障接管只能等 TTL（默认 300s）。5 分钟的接管延迟对生产服务偏长。
